		...
		if (State->ForwardSimulating) {
			...
			if (CoastDurationSeconds > 1.0f){ 
				if (State->ForwardSimulatingTimeRemaining < SecondsToMilliseconds(CoastDurationSeconds)) {
					...
					// Forward simulation ends after ship finishes coasting
...
void ShipSelected(selection* Sel, engine_state* EngineState, game_input* Input, i32 SelectionIndex)
{
	...
	if (!CurrentShip->CreatingMovement) {
		...
		if (!CurrJour->InProgress) {
			if (ImGui::Button("Move Ship", ImVec2(-1, 0))) {
				CurrentShip->CreatingMovement = true;
				State->AnyShipCreatingMovement = true;
			}
		} else {
